...
function BarberLayoutContent({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-primary-fixed-dim/30 border-t-primary-fixed-dim rounded-full animate-spin"></div>
          <p className="text-outline-variant uppercase tracking-widest text-xs">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <BarberSidebar />
      <main className="lg:ml-72 flex-1 min-h-screen flex flex-col relative">
        {children}
      </main>
    </div>
  );
